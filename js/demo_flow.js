...
var dataFile = "https://raw.githubusercontent.com/KaiboLiu/HualuCup/master/data/try.dat";
    ...
    {   
        ...
        //for (var lineNo=8; lineNo<9; lineNo++){
            ...
            for (var stopNo=1; stopNo<n_stops[lineNo]; stopNo++){
                ...
                var newline_up   = getNode('line', {x1: p1.x-dx, y1:p1.y+dy, x2:p2.x-dx, y2:p2.y+dy, 
                                                    stroke:traffic_color_up,   strokeWidth:sideWidth, strokeDasharray:dashLength+" "+dashLength, strokeDashoffset:dashLength*2,
                                                    class:"tooltips-seg line_up"+lineNo, id:"line"+(lineNo)+'_up:'+stopNo+':['+names[lineNo][stopNo-1]+'-->'+names[lineNo][stopNo]+']'});
                var newline_down = getNode('line', {x1: p1.x+dx, y1:p1.y-dy, x2:p2.x+dx, y2:p2.y-dy, 
                                                    stroke:traffic_color_down, strokeWidth:sideWidth, strokeDasharray:dashLength+" "+dashLength, strokeDashoffset:dashLength*2,
                                                    class:"tooltips-seg line_down"+lineNo, id:"line"+(lineNo)+'_down'+stopNo+':['+names[lineNo][stopNo]+'-->'+names[lineNo][stopNo-1]+']'});
                var newline      = getNode('line', {x1: p1.x,    y1:p1.y,    x2:p2.x,    y2:p2.y, 
            ...
            for (var stopNo=0; stopNo<n_stops[lineNo]; stopNo++){
                ...
                var newdot = getNode('circle', {cx:p1.x, cy:p1.y, r:4, 
                                    stroke:color_dot_stroke, fill:color_dot_fill, strokeWidth:dot_stoke_wid,
                                    class:"tooltips-stop line"+lineNo, 
                                    id:"line"+(lineNo)+"_stop"+stopNo+':'+names[lineNo][stopNo]+' ('+p1.x+','+p1.y+')',//});
...
function DC_fillTable(n_line, n_stops, names, time_up, time_down, lineNo, stopNo){
    ...
    for (var i=0; i<highlightSegs.length; i++){
        highlightSegs[i].style.stroke = 'yellow';
    }

    var tableDiv = document.getElementById('table_div');
    var title = document.createElement('h1');
    title.innerHTML = '<br>公交'+(lineNo)+'路：由<span style="color:blue;font-size:110%;">'+names[lineNo][stopNo]+'</span>站出发时间';
    ...
    for (target = stopNo-1; target >= 0; target -= 1){    // i --> i+1,..., i --> n
        if (time_down[lineNo][target-1] < 0){
            time += 1
        } else {
            time += time_down[lineNo][target-1];
        }
